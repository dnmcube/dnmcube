...
internal class BookAccessor<T> : AccessorAbstarct<T>, IAccessor where T:Book
{
    private CRUD<T> _crud;
    public BookAccessor(ILifetimeScope lifetimeScope) : base(lifetimeScope)
    {
    ...
    private void Query(UserFilter filter)
    {
        if (filter.Id != null)
        {
            Querys = _userRepo.GetQueryable(x => x.Id == filter.Id).Include(x => x.User);
            return;
        }
